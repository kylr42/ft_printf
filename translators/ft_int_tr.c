...
static void	ft_ralignment(t_node *node, long num)
{
	while (node->width-- > 0)
		ft_putchar_ln(' ', node);
	if (num < 0)
...
static void	ft_lalignment(t_node *node, long num)
{
	if (num < 0)
		ft_putchar_ln('-', node);
	while (node->precision-- > 0)
...
static void	ft_costyli(t_node *node, long num)
{
	int		len;
	...
	if (num < 0 || (node->dot == -1 && num == 0))
		node->width--;
	if (node->flag == '0' && node->dot == -1 && node->width > 0)
	{
		node->precision = node->width;
		node->width = 0;
	}
	if (node->dot < -1 && num == 0 && node->width > 0)
		node->width--;
	if (node->flag == '0' && node->dot < -1 && node->width > 0)
	{
		node->precision = node->width;
